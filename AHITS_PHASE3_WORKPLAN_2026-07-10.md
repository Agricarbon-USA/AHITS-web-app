...
## 15. WORKSTREAM 7 — The pilot floor (2026-07-28 re-baseline; INSERTED AHEAD of the capstone queue)

> **Added 2026-07-28 by the six-seat critical review + owner interview** (`AHITS_SIX_SEAT_REVIEW_2026-07-28.md` — findings with file:line evidence; `DECISIONS.md` D16–D20 — the five decisions). The pilot start was postponed to a **rolling start** (D17), which converts launch-week firefighting into a pre-launch hardening window. **This workstream gates the first operator onboarding** and re-orders — but does not re-argue — the capstone queue.

- **W7-1 · CC-29 Offline trust floor** [FUN] — the queue's write-loss P0s: photo-wedge, lie-fi timeout, online-401 parking, late-sync date semantics (replayed checks keep their original business date; past-date collisions 409, never merge), sliding session renewal (14-day cap), concurrent-flush single-owner, dead-localphoto rejection, and the queue's first real test harness. **L. GATES FIRST OPERATOR.** → `AHITS_CC29-31_PILOT_FLOOR_PACKETS.md`
- **W7-1b · CC-32 Friction & flow (the operator-experience floor)** [SIM][FUN] — added later the same session on Max's UX-priority call, slotted directly after CC-29: the D11 glossary sweep (one word per state, Max confirms the word list first), one-time failure entry, site prefill, optional handoff note with presets, the 4→2-step deployment builder, the second-check/scan-path template-race fix (Antagonist sign-off), GPS warm-capture at the review step, the pickup-hold deadline, an openable Outbox for pending items, the copy-honesty pass (no more "pull to refresh" on iOS; non-misdirecting empty states), Map in the operator bottom nav + crew-map offline caching, and the 44px pass on the Today components. Every changed interaction states taps-added vs taps-removed in the PR body. **M–L.**
- **W7-2 · CC-30 Staging is home** [FUN][TRA] — D16 mechanics: preview-deploy path killed, migration-safety on the live branch, seed/reset guards, pilot env in the pipeline (min-instances=1, EMAIL_SANDBOX_TO), Sentry lost-write feed + cron un-silencing, `PILOT_ROLLBACK.md` + reconciliation query pack, and Max's never-attempted live-fire checklist (backups/PITR + restore drill, healthchecks, CRON_SILENT, Sentry, branch protection, Mapbox restriction). **M–L.**
- **W7-3 · CC-31 Accuracy floor + /admin/pilot** [FUN][SPK] — INV-5 false-alarm fix + auto-resolves for the linger-forever alerts (the bell must stay trustworthy), expired HUB_RETURN unjam, forOperatorId scope, durationMs excluding the GPS wait (charter metric 2 honesty), the pilot-metrics dashboard (adoption + duration distribution + GPS grant rate + pencil-whip flags + viewer deep-links), per-day denominator snapshots, request-scope indexes. **L.**
- **W7-4 · CC-16S Public-surface security** [FUN] — the CC-16 step-0 split (D18): dead `s/[token]` links stop serving live inventory; stable per-line idempotency keys. CC-16-proper **PARKED** with a named trigger. **S–M.**
- **W7-6 · CC-33 Simplify & unify (post-CC-16S)** [SIM] — D21/D22/D23 mechanics: the verified dead-code sweep (~450 LOC + 3 deps, incl. `lib/shipments.ts`; `Shipment` table stays dormant per D23), Forward→Operator branch removal, batch6a residue landed by hand, and the unified "Transfer" entry (Entire rig = handoff semantics, ownership flips, plain-words consequence copy; Selected gear = transfer) — client-flow only, server models untouched, new components in new files per D21's anti-regrowth rule. **M.**
- **W7-5 · Non-code riders (owner):** Stewart's ADMIN account + the one-page admin-verbs guide (D20); the onboarding ritual as a one-pager anyone can run (D17 — rolling start means no single launch morning); baseline the group-text volume before first operators go live (the adoption question's missing control arm); a "someone gets hurt" line on the triage card; **the mounted-units template bridge (D24 — add Wintex/Giddings items to the Can-Am/Bobcat checklist templates via the admin editor, ~30 min, zero code)**; **the Clockify archive snapshot (D19 rider — full CSV export + measure its real daily tap count while the incumbent is still alive)**.

**Queue changes downstream (recorded, not re-arguable without a superseding D):** CC-16-proper OUT of the live queue (D18) → the **secondary-operator Today packet** (review §3.2 — three-answers seam; deferred because this cohort is all solo-PRIMARY, trigger = any shared rig OR pre-CC-17) and the **D11 glossary sweep** (strings-only, urgency raised by review §4.3) slot next (the D11 sweep is CC-32 PR-1, not a standalone) → **CC-17 as FULL REPLACE of Clockify with D19's riders** (measure Clockify's daily taps before the design meeting; sequence the invoice pipeline early — the invoicing pain is the wedge; earnings view stays in DoD; full A6 matrix + D3 + FND-8 gates unchanged) → CC-18 (its customer is Max himself, per interview — re-confirm scope before committing the quarter). **Pilot readout additions (charter §2 riders):** who initiates opens, did the group text shrink vs baseline, `createdAt` vs `date` right-time analysis, interventions-per-operator, and the exit question — asked as a conversation.

**Closed/retired by the 2026-07-28 second interview pass:** CC-21 mounted-units spike CLOSED (D24 — no model; template config + carrier-vehicle maintenance tasks; the L build is cancelled); NS-1 evidence probe RETIRED (D25 — audits flow through the science-side app; lane = passive capture); NS-9 trigger MET (D23 — minimal shipment tracking queues post-CC-17); Forward→Operator removed + portal reservation-channel frozen (D21). **Post-CC-17 queue additions:** shipment tracking (D23), onboarding-funnel card on /admin/pilot (S, ride-along after CC-31), NS-4 weather stamps as a CC-17 rider with one-shot GPS backfill, EmailLog resend + invoice-kind alert as CC-17 acceptance riders, N-7 spec lines (TimeEntry carries projectId/rigId; Settings model home) into the CC-17 design meeting.

**Corpus-thinning (accepted in principle, execute lazily):** Master Roadmap + workplan-v2 freeze as reference (banner-patch them no further); STATUS §1 thins to pointers at the next natural rewrite; the CLAUDE.md-vs-STATUS session-close lists collapse to one (keep step 7); ID namespaces frozen (no new ones); review seat-count scales with blast radius (CC-17 gets all six; strings-only sweeps get one adversarial pass).
