...
2. **FND-6 final shard** — `src/app/s/[token]/page.tsx`. The per-line `Idempotency-Key` embedded `Date.now()`, so double-tap dedup never fired. **Fix:** a per-intent random nonce in a ref (`lineActionNonce`) keyed by `(lineId, action)` — minted once, reused for every retry/double-tap of that intent (dedup fires), **re-minted on any server-settled response** so the next intent (re-editing the same line to a new qty, OR correcting a bad qty after a cached 400) gets a fresh key and applies. **Antagonist fix (Max, mid-session):** the first cut cleared on success only; but `withIdempotency` caches 400s bound to the body hash, so holding the nonce meant a corrected retry 422'd ("already used with a different request") forever. Now held ONLY across network errors and the transient in-flight 409 (`IDEMPOTENCY_IN_FLIGHT_ERROR` — the original write may still land, so a retry must reuse the key). Random (not a counter) so a page reload can't collide with a prior different-body key.
