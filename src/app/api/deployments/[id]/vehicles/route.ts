...
const schema = z.object({
  vehicleIds: z.array(z.string()).min(1, 'At least one vehicle required'),
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { vehicleIds, note, photoUrls } = parsed.data
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await prisma.$transaction(async (tx) => {
    await tx.rigVehicle.updateMany({
      where: { rigId: id, vehicleId: { in: vehicleIds }, removedAt: null },
      data: { removedAt: now, removeNote: note },
    })
    await tx.vehicle.updateMany({
      where: { id: { in: vehicleIds } },
