...
Version 2.5 · prepared 2026‑06‑22 · supersedes `AHITS_PRD_v1`, the June "Post Wave 0" status update, and PRD v2.0/v2.1/v2.2/v2.3/v2.4
...
- **v2.4 (this revision)** — brings the document current through a **fourth build session** (assessment-driven **Wave A** hardening). Shipped to `development` and **verified on staging**: the **correctness cluster** (C1 consumable/serialized check-out race + silent over-promise, H1 vehicle double-assignment between rigs, H4 daily-check ownership, H5 floating/unreliable alert) with the first deployment-route regression tests (`tests/wave-a-correctness.test.ts`); **A1 CI/deploy gating** — a shared reusable `verify` workflow (lint / type-check / build / tests) now gates every PR *and* every deploy, with concurrency guards and the `develop`→`development` trigger fix; and a **DAT-5/DAT-7 drift reconciliation** that resolved a production-blocking staging outage (the `itemType`/`ResolutionPath` enums and `Alert.activeKey` had been `db push`ed onto the shared database and deployed but never captured in a migration or merged to `development`). Built and **PR-pending**: **A3** (`/api/health` + Cloud Run startup probe) and the **A2 redo** (automated migrate-on-deploy via the Supabase IPv4 **session pooler**, after the first attempt was blocked by Supabase's IPv6-only direct connections vs IPv4-only GHA runners). Also surfaced and partially resolved: the production database lived in the **contractor's Supabase org** (now Owner-shared with Max; full transfer pending). Full detail — branch/PR/migration map, the drift narrative, and the carry-forward backlog — is in companion `AHITS_SESSION4_ANALYSIS_AND_PLAN.md` §9.
- **v2.5 (this revision)** — brings the document current through a **fifth build session**, all merged to `development` and **staging-verified**. Shipped: **UX-6** (offline end-deployment / bulk-return through the queue; transfers blocked-with-message offline) + **CR-1** (consumable draw-down on checkout / restore on good returns, via a `consumed` flag) [PR #39]; **Wave B — photos end-to-end** (camera capture + client-side compression + offline-blob upload-on-reconnect; closes the long-absent photo pipeline) [PR #40]; **Wave E — the admin Maintenance inbox** (the destination the operator→admin damage loop never had) + **UX-12** alert deep-links [PR #42]; **Wave F — the notification dispatcher** (cron-driven email + in-app notification center, idempotent) + **CR-5** alert-dedup fix + the A3 `/api/health` endpoint [PR #43/#44, with `AHITS_CRON_SECRET` + Cloud Scheduler now live]; and **Wave G — maintenance recurrence** (scheduled tasks roll forward; damage repairs return the unit to service; daily-check odometer drives mileage-based due/overdue). A subsequent **consolidation audit** fixed a dozen correctness/cohesion issues (partial-transfer double-decrement, offline-photo partial-batch loss, notification double-send, cron auth hardening, et al.) — full ledger in companion **`AHITS_SESSION5_RECORD.md`**. The field→admin loop is now closed end to end; the live backlog is **`AHITS_CONSOLIDATED_TRACKER.md`**.
...
**Where the build stands now (as of v2.5).** Session 5 closed the central gap the assessment kept flagging: **the field→admin loop is now wired end to end.** Photos are captured (offline-capable) and persisted on damage reports; the **admin Maintenance inbox** (Wave E) receives them; dashboard alerts **deep-link** to the task; and the **notification dispatcher** (Wave F — live on Cloud Scheduler) emails admins and populates an in-app notification bell. Maintenance now **recurs on its own** (Wave G): scheduled tasks roll forward on completion, damage repairs return the unit to service, and daily-check odometer readings drive mileage-based due/overdue. A consolidation audit then hardened the new code (offline-photo durability, notification idempotency, transfer accounting, cron auth). The remaining work is concentrated in **pilot-readiness infrastructure** (a real production database with its own secrets; migrate-on-deploy automation; the A6 real-device offline pass), **outbound completion** (external delivery to maintenance shops + the invoice→processor loop; hub receive/return-receipt), and **admin completeness/polish** (the Vehicles/Projects/Reports stubs + utilization report; the Wave D security tail). Per the canonical scheme, the wave numbering is now **A–H + Phase 3**; the live status doc is `AHITS_CONSOLIDATED_TRACKER.md` and the narrative session record is `AHITS_SESSION5_RECORD.md`.
