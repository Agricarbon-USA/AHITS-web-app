> STATUS: superseded   ·   UPDATED: 2026-06-30
> SUPERSEDES: —   ·   SUPERSEDED-BY: AHITS_PHASE3_WORKPLAN_v2.md (root)
> READ-WITH: AHITS_PHASE3_WORKPLAN_v2.md
> NOTE: Predates decisions D1 (prod deferred), D2 (route history in scope), D3 (money loop un-gated on DROP); read v2 + DECISIONS.md for current state.
...
**Out of scope (the one map dimension we deliberately don't build).** Real-time / continuous tracking — the map updates **only** on daily-check submit; no live GPS stream, background location, or polling. Everything else formerly parked here is now folded into scope below.

**Folded-in extensions.** Each rides the same opt-in, check-submit GPS capture (no new capture path, no real-time tracking) and is read-side over data the base map already persists.

- **M-1 · Path history (check-to-check breadcrumb).** Connect a single deployment's ordered daily-check pins into a breadcrumb so the manager can see where it has *been* over its run, not just its latest position. **Data:** none new — query all of a deployment's `DailyCheck` rows carrying coords, ordered by `businessDate`. **UI:** click a deployment → draw a numbered polyline through its check-pins (days without a fix are simply skipped); the latest pin keeps its recency color. **Explicitly not** a continuous GPS trace (that is the real-time line above). **Acceptance:** selecting a deployment shows its pins connected in `businessDate` order; gaps skipped; toggling off returns to the single-pin view. **Effort:** small (read-side; data already captured). **Risk:** low.

- **M-2 · Operator-facing map.** Give operators a read-only map instead of no map. **Default scope (build):** the operator sees *their own* active deployment pin + its M-1 path + read-only site/access context ("where am I, where have I been") — a genuine operator-value surface. **Peer visibility (gated decision — do NOT build by default):** showing *other* operators' pins collides with the North Star's explicitly-declined "crew-to-crew visibility / where's everyone else" anti-goal (surveillance-adjacent). It ships only on an explicit product decision that overrides that anti-goal, and if taken, scoped to same-project/same-hub peers rather than the whole fleet. **Data/API:** reuse the admin map aggregate, filtered server-side to the requesting operator. **Acceptance:** an operator opens a map showing their deployment + path; no other operator's location is visible unless the peer-visibility decision is made and recorded. **Effort:** small (own-only) / medium (gated peer layer). **Risk:** low, plus the anti-goal flag above.

- **M-3 · Pending-pickup pins (REQUESTED/STAGED reservations).** Surface staged/awaiting-pickup work on the same map so the manager sees not just what's *running* but what's *ready to go*. **Data:** none new — `DeploymentRequest` (type RESERVATION, status REQUESTED/STAGED) pinned at its fulfiller **hub** location. **Dependency (flag):** hubs carry address fields (`street1/zip/country`) but not lat/lng today — either add nullable `Hub.lat/lng` (additive, captured at hub setup) or geocode the address server-side once and cache; pick one before build. **UI:** a distinct pin style/color ("pending pickup") in a second layer; tooltip → the request; the pin clears when the reservation becomes an active deployment. **Acceptance:** REQUESTED/STAGED reservations appear as distinct pins, link to the request, and disappear on fulfillment. **Effort:** small-medium (the hub-geocoding decision is the only real cost). **Risk:** low-medium.

**Sequencing note.** M-1 and M-3 are read-side and can ride the base Map capstone directly; M-2's own-only view is small and follows immediately, with its peer layer gated on the anti-goal decision. None of the three gates the base map's ship.

**Acceptance criteria (base map).** Opt-in prompt on submit; denial still submits; coords persist and survive offline replay; pins render with correct recency colors; tooltip links resolve; CSP `connect-src`/`img-src` updated for Mapbox tiles. **Tests:** payload-with-gps round-trip; denied-permission submit path; recency-bucket color logic. **Effort:** small. **Risk:** low — additive, de-risks the GPS plumbing the analytics later reuse.
