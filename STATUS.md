...
_Last updated: **2026-07-11**. If this date is more than a session old, trust the code and `git log` over this file, and update it._
...
AHITS is at the pilot doorstep, running on **staging**. The hard architecture is done and audit-verified. This session established the Phase-3 plan of record, ran a six-seat code audit, and began landing the trust-floor fixes. Merged to `development` (→ staging) so far: the release-safety hardening, the data-integrity cluster, the offline/trust one-liners, and the dashboard-fix + BOBCAT-enum packet. The mobile quick-wins packet is in flight. Production is deliberately deferred (see D1).
...
- **CC-07** (mobile quick-wins triage) — branch `feature/…mobile-quick-wins`, pushing/smoking → merge next.
- **A6 device pass** — the pilot gate; human-run on real iOS + Android hardware; **run in parallel now** (not started).
- Documentation self-navigating cleanup (this doc + `DECISIONS.md` + `00_START_HERE.md` + dedup + banners) — in progress.

## 4. Next actions (ordered)
1. Finish/merge **CC-07**; run the **A6 device pass** in parallel.
2. Land the three pending Wave-0 patches (EmailLog-FAILED alert, URL-filters rollout, date-unify) — *verify their current merge state first.*
3. **Wave B:** CC-08 (Hubs discrepancy + bulk verify) → CC-09 (Awaiting-Pickup) → CC-10 (field-fix log) → CC-11 (admin-as-operator, built on `deployment_assignments`, money-loop excluded per D3). CC-08's only real dependency (CC-02) is merged.
...
- **Decisions:** `DECISIONS.md` · **Latest handoff:** `AHITS_SESSION_HANDOFF_2026-07-10.md` · **Rules for changing code:** `CLAUDE.md`
