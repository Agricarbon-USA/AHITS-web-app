...
| CC-34 | Maintenance speaks (D29) | R3 drawer health + rig/reporter, one report verb (R2) + check→task promotion, **schedules become real** (Add-scheduled-task UI = D24 bridge, stale-damage nag, DEPLOYMENT close + stranded-unit fix, operator "In repair", WORK_ORDER-completed alert). D29 recorded; **PER_DEPLOYMENT UI parked** (D29-4 — cut from the new form; zod enum + schema dormant; revisit = a real "service every N deployments" demand) | PR-1 #226 + PR-2 #227 ✅ MERGED; **PR-3 #229 (schedules) OPEN + full CI green** — owed: Max's pre-merge stale-damage SELECT vs staging + evening smoke, then merge. Mark fully landed only after #229 merges |
