...
| CC-33 | Simplify & unify | Dead-code sweep, Forward→Operator removal (D21), one unified Transfer entry (D22); **batch6a date-unify landed by hand (PR-1)** | ✅ MERGED — #223 (dead-code + forward-removal) + #224 (unified Transfer), both staging deploys green. D21/D22 EXECUTED. Owed: Max's two-phone Transfer smoke on live staging |
| CC-34 | Maintenance speaks (D29) | R3 drawer health + rig/reporter, one report verb (R2) + check→task promotion, **schedules become real** (Add-scheduled-task UI = D24 bridge, stale-damage nag, DEPLOYMENT close + stranded-unit fix, operator "In repair", WORK_ORDER-completed alert). D29 recorded; **PER_DEPLOYMENT UI parked** (D29-4 — cut from the new form; zod enum + schema dormant; revisit = a real "service every N deployments" demand) | ✅ MERGED — PR-1 #226 + PR-2 #227 + **PR-3 #229 (schedules)**, post-merge `deploy.yml` green end-to-end (tip `901f413`). No migration in PR-3. Residual (verification): Max's staging smoke of 3a/3c/3d + the stale-damage SELECT (never run pre-merge; cron arm bounded 5 nags/run) |

> **Landing order superseded by the 2026-07-28 re-baseline (STATUS §4 / D17):** the live queue is **CC-29 (✅) → CC-32 (✅) → CC-30 (✅) → CC-31 (✅) → CC-16S (✅ #220/#221) → CC-33 (✅ #223/#224) → CC-34 (PR-1 #226 + PR-2 #227 ✅ MERGED; PR-3 #229 schedules OPEN + green)** (CC-34 pastes with RIDER C in `AHITS_PACKET_ERRATA_2026-07-29.md`). The CC-16 "NEXT" flag above is stale — CC-16 was split (CC-16S ships the step-0 security now; CC-16-proper is PARKED, D18).
