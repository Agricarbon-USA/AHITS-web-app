...
import { listRequests } from '../src/lib/deployment-requests'
...
describe('NEW-3: admin creates a request assigned to an operator', () => {
  ...
  // CC-31 item 4 (review §3.6): the operator a request was filed FOR (forOperatorId)
  // now sees it in listRequests (Today + Requests page) — closing the split where an
  // Awaiting-Pickup card / hold-TTL notice pointed at a Requests page the request had
  // never appeared on. An unrelated operator still doesn't see it.
  it('listRequests(X) includes an admin-filed forOperatorId=X request; listRequests(Y) does not', async () => {
    const admin = await createAdminUser()
    const x = await createOperator()
    const y = await createOperator()
    const hub = await createHub()
    const cat = await createCategory()
    mockSession = adminSession(admin.id)
    ...
    const created = await prisma.deploymentRequest.findFirst({ where: { label: 'CC31-for-X' } })
    expect(created?.forOperatorId).toBe(x.id)

    const forX = await listRequests(x.id)
    expect(forX.some((r) => r.id === created!.id)).toBe(true)
    // and the row carries requestedById (the admin) so the client can keep it read-only for X
    expect(forX.find((r) => r.id === created!.id)?.requestedById).toBe(admin.id)

    const forY = await listRequests(y.id)
    expect(forY.some((r) => r.id === created!.id)).toBe(false)
  })
})
